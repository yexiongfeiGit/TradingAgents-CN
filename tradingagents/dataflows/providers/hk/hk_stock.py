...
提供港股数据的获取、处理和缓存功能，支持通过Yahoo Finance获取港股实时和历史数据。

主要功能：
1. 历史数据获取：支持指定日期范围的港股历史K线数据
2. 实时价格获取：获取港股的实时价格信息
3. 股票信息获取：获取港股的基本信息（名称、市值、行业等）
4. 代码标准化：自动标准化港股代码格式（如0700.HK）
5. 频率限制：智能的请求频率控制和重试机制
6. 错误处理：完善的异常处理和重试策略
7. 缓存支持：支持数据缓存和重复利用
8. 多格式支持：支持多种日期格式和代码格式

数据格式：
- 历史数据：pandas DataFrame格式，包含OHLCV数据
- 实时价格：字典格式，包含当前价格、开高低收、成交量等
- 股票信息：字典格式，包含名称、货币、交易所、市值、行业等

代码标准化：
- 自动识别：支持多种港股代码格式
- 统一转换：自动转换为Yahoo Finance标准格式
- 格式验证：验证代码格式的有效性
- 错误处理：无效代码格式的友好处理

频率控制：
- 最小间隔：可配置的最小请求间隔（默认2秒）
- 指数退避：失败时的指数退避重试策略
- 频率限制：检测到频率限制时的智能等待
- 最大重试：可配置的最大重试次数（默认3次）

配置参数：
- TA_HK_MIN_REQUEST_INTERVAL_SECONDS：最小请求间隔（默认2秒）
- TA_HK_TIMEOUT_SECONDS：请求超时时间（默认60秒）
- TA_HK_MAX_RETRIES：最大重试次数（默认3次）
- TA_HK_RATE_LIMIT_WAIT_SECONDS：频率限制等待时间（默认60秒）
...
使用示例：
    ...
    # 创建港股数据提供器
    provider = HKStockProvider()
    
    # 获取腾讯控股的历史数据
    data = provider.get_stock_data("0700", "2024-01-01", "2024-01-31")
    
    # 获取实时价格
    price_info = provider.get_real_time_price("0700")
    
    # 获取股票基本信息
    info = provider.get_stock_info("0700")
