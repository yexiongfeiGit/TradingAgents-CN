...
该模块实现了专业的社交媒体情绪分析智能体，负责监控和分析
中国主要财经平台的投资者情绪变化。智能体通过多数据源整合
提供全面的市场情绪评估和影响力分析。

主要功能：
- 多平台情绪监控：雪球、东方财富股吧、财经新闻等
- 投资者情绪量化：情绪指数评分和变化趋势分析
- KOL影响力分析：关键意见领袖观点追踪
- 热点事件影响：政策变化和市场传言情绪影响
- 情绪-价格关联：情绪变化对股价的预期影响

智能体特性：
- 多市场支持：A股、港股、美股公司名称智能识别
- 统一数据源：集成多个社交媒体和财经平台数据
- 情绪量化指标：1-10分情绪指数和波动幅度预测
- 防循环机制：限制工具调用次数避免死循环
- 专业分析报告：结构化输出情绪分析结果
...
def create_social_media_analyst(llm, toolkit):
    ...
    该工厂函数创建专业的社交媒体情绪分析智能体，负责监控
    中国主要财经平台的投资者情绪变化并生成情绪分析报告。

    Args:
        llm: 语言模型实例，用于生成情绪分析报告
        toolkit: 工具包实例，包含情绪数据获取工具
        
    Returns:
        function: 社交媒体分析师节点函数，接收状态并返回更新后的状态
    ...
    def social_media_analyst_node(state):
        ...
        该节点负责执行社交媒体情绪分析任务，工作流程：
        1. 检查工具调用次数，防止死循环（最大3次）
        2. 识别股票类型和市场信息
        3. 获取公司名称（支持多语言智能识别）
        4. 调用统一情绪分析工具获取社交媒体数据
        5. 生成结构化情绪分析报告
        
        状态更新：
        - messages: 添加分析消息和工具调用结果
        - sentiment_report: 存储最终情绪分析报告
        - sentiment_tool_call_count: 记录工具调用次数
