...
该模块实现了专业的交易决策智能体，负责综合分析各类市场信息
并生成具体的买入、卖出或持有建议。智能体具备以下核心功能：

主要功能：
- 多市场支持：自动识别A股、港股、美股，使用对应货币单位
- 综合分析：整合市场报告、情绪报告、新闻和基本面分析
- 历史记忆：利用过往交易经验避免重复错误
- 风险控制：提供置信度、风险评分和目标价位
- 专业决策：生成结构化的投资建议报告

智能体特性：
- 货币智能适配：人民币(¥)、美元($)、港币(HK$)
- 目标价位强制要求：必须提供具体数值，不允许null
- 历史经验学习：基于过往相似情况的交易反思
- 多维度分析：结合技术、基本面、情绪、新闻因素
...
def create_trader(llm, memory):
    ...
    该工厂函数创建专业的交易决策智能体，负责综合分析各类市场信息
    并生成具体的交易建议。智能体具备多市场支持和历史记忆功能。

    Args:
        llm: 语言模型实例，用于生成交易建议
        memory: 记忆系统，用于获取历史交易经验和反思
        
    Returns:
        function: 交易员节点函数，接收状态和名称参数，返回更新后的状态
        
    智能体工作流程：
        1. 获取各类分析报告（市场、情绪、新闻、基本面）
        2. 识别股票类型和货币单位
        3. 检索历史相似交易经验
        4. 构建综合分析上下文
        5. 生成结构化交易建议
