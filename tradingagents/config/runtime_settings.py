#!/usr/bin/env python3
"""
TradingAgents 运行时配置适配器模块（弱依赖）

提供动态配置获取功能，支持从后端系统设置和环境变量中读取配置，
保持TradingAgents包的独立性，在不可用时静默回退到环境变量和默认值。

主要功能：
1. 动态系统设置获取（优先于环境变量）
2. 事件循环冲突检测和处理
3. 多类型配置值解析（布尔型、整型、浮点型、时区）
4. 缓存控制配置管理
5. 配置来源追踪和日志记录

配置优先级：
1. 数据库动态系统设置（system_settings）
2. 环境变量
3. 代码默认值

特性：
- 弱依赖：后端不可用时静默回退，不引入硬依赖
- 事件循环安全：检测并避免事件循环冲突
- 类型安全：提供类型注解和运行时类型检查
- 配置追踪：记录配置来源和评估过程
- 向后兼容：支持多种环境变量命名格式
...
支持的配置类型：
- 数值类型：整数、浮点数
- 布尔类型：支持多种格式（true/false、yes/no、1/0等）
- 字符串类型：时区名称等
- 缓存控制：应用缓存启用状态

使用示例：
    # 获取数值配置
    interval = get_float("TA_US_MIN_API_INTERVAL_SECONDS", "ta_us_min_api_interval_seconds", 1.0)
    
    # 获取布尔配置
    cache_enabled = use_app_cache_enabled(True)
    
    # 获取时区配置
    timezone = get_timezone_name("Asia/Shanghai")
